...
import type { FaceInput } from "@rathodparesh/face-verify-core";
...
export function FaceVerificationStudio({ mode = "enrollment", referenceEmbedding, showQuality = true, showJson = true, threshold = 0.72, includeLandmarksInJson = false, includeEmbeddingInJson = true, onEmbeddingCreated, onEnrollmentComplete, onVerificationComplete, onError, ...captureProps }: FaceVerificationStudioProps) {
  ...
  return <div className="fv-studio"><div><FaceCapture {...captureProps} {...(onError ? { onError } : {})} onCapture={({ source }) => void process(source)} /><button type="button" className="fv-reset" disabled={!capture && !value} onClick={() => { api.reset(); setCapture(undefined); }}>Reset</button></div><aside>{api.isProcessing && <p role="status">Processing face locally…</p>}{api.error && <p role="alert">{api.error.message}</p>}{showQuality && <FaceQualityPanel quality={api.enrollmentResult?.quality} />}<VerificationResult result={api.verificationResult} />{showJson && value && <JsonViewer value={value} filename={`face-${mode}-${Date.now()}.json`} />}</aside></div>;
}
